...
class OpenCCSSpawner(BatchSpawnerRegexStates):
    ...
    ssh_cmd = " ".join(["ssh", "-i", "/home/tunnelbot/.ssh/id_rsa", "juphuser@131.234.58.16", "env"] + [val+"=$"+val for val in env_var.split(",")]) + " "                   
    batch_submit_cmd = ssh_cmd + ' /scratch/hpc-lco-jupyter/jh_ccsalloc'
    batch_query_cmd = ssh_cmd + ' ccsinfo -s --mine --fmt=%m%w --raw {job_id} | tr ":" " "'                                                                 
    batch_cancel_cmd = ssh_cmd + ' ccskill {job_id}'
    state_exechost_re = '(^node[0-9][0-9]-[0-9][0-9][0-9])|(^gpu[0-9][0-9][0-9])'
    state_pending_re = r'PLANNING|PENDING'
    state_running_re = r'ALLOCATING|ALLOCATED'

    batch_script = """#!/usr/bin/bash
    #CCS --res="rset=1:ncpus={{nprocs}}:mem={{memory}}"                                                  
    #CCS -t {{runtime}}
    #CCS -N jhub_{{username}}

    # SETTING TEMP WORKDIR
    export HOME=/scratch/hpc-lco-jupyter/
    rootd=/scratch/hpc-lco-jupyter/HOME_DIRECTORIES/$JUPYTERHUB_USER/$CCS_REQID                                                                                              
    module load lang/Python/3.6.6-foss-2018b
    export LD_LIBRARY_PATH=/scratch/hpc-lco-jupyter/jupyterhub/lib/python3.6/site-packages:$LD_LIBRARY_PATH                                                                  
    export JUPYTERHUB_API_URL="http://194.13.83.158:8081/hub/api"

    # FILL TEMP WORKDIR
    hostname > $rootd/running_host

    {{cmd}}
    """

    def parse_job_id(self, output):
        # OUTPUT OF JUB SUBMISSION IS SOMETHING LIKE: 5074915 (because of --quiet)
        return output
